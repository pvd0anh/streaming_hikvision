...
class VideoTransformTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        if self.transform == "Pose estimator":
            img = frame.to_ndarray(format="bgr24")
            _, buffer = cv2.imencode('.jpg', img)
            bbox = ''
            request = dl_server_pb2.DLImageTaskRequest(task_name="Pose estimator", image=buffer.tobytes(), bbox=bbox)
            response = STUB.proceed_image_task(request)
            img = cv2.imdecode(np.frombuffer(response.image, np.uint8), 1)
        ...
        elif self.transform == "Counting time":
            print(frame.shape)
            return frame
...
async def index(request):
    content = open(os.path.join(ROOT, "templates/test.html"), "r").read()
